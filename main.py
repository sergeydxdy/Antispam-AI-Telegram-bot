...
subscribed_chat_statuses = {'-1002202582670': 'basic'}
...
class Bot:
    def __init__(self, api, ai_model):
        self.api = api
    ...
    def _send_message_chat(self, chat_id, reply_to_message_id):
    ...
    def _send_message_private(self, chat_id, text):
        url = self.base_url + 'sendMessage'
    ...
    def _process_group_messages(self):
        ...
        for update in updates.get('result', []):
            ...
            if message and 'text' in message and message['message_id'] not in self.processed_messages:
                ...
                if subscribed_chat_statuses.get(str(chat_id)) == 'basic':
                    if self.model.predict_spam(text=message['text']):
                        self._delete_message(chat_id, message['message_id'])
                        self.processed_messages.append(message['message_id'])

    def _process_private_messages(self):
        updates = self._get_updates()
        for update in updates.get('result', []):
            self.last_update_id = update['update_id']
            message = update.get('message')
            if message and 'text' in message:
                ...
                if text.lower() == 'привет':
                    self._send_message_private(chat_id, 'Привет')

    def run(self):
        while True:
            self._process_group_messages()
            self._process_private_messages()
